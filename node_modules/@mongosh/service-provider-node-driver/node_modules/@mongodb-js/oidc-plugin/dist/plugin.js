...
function tokenExpiryInSeconds(tokenSet = {}, passIdTokenAsAccessToken = false, now = Date.now()) {
    ...
    const expiresAt = (tokenSet.id_token &&
...
function automaticRefreshTimeoutMS(tokenSet, passIdTokenAsAccessToken = false, now = Date.now()) {
    ...
    if (!tokenSet.refresh_token || !expiresIn)
        return;
...
class MongoDBOIDCPluginImpl {
    ...
    updateStateWithTokenSet(state, tokenSet) {
        ...
        const refreshTokenId = (0, util_1.getRefreshTokenId)(tokenSet.refresh_token);
        ...
        const tryRefresh = (0, util_1.withLock)(async () => {
            ...
            if (state.currentTokenSet?.set !== tokenSet) {
                this.logger.emit('mongodb-oidc-plugin:refresh-skipped', {
                    ...
                    actualRefreshToken: (0, util_1.getRefreshTokenId)(state.currentTokenSet?.set?.refresh_token),
                });
                return false;
            }
            try {
        ...
        this.logger.emit('mongodb-oidc-plugin:state-updated', {
            updateId,
            timerDuration,
            tokenSetId: (0, util_1.getStableTokenSetId)(tokenSet),
        });
    }
    async authorizationCodeFlow(state, signal) {
        ...
        const nonce = this.options.skipNonceInAuthCodeRequest
            ? undefined
            : openid_client_2.generators.nonce();
        try {
                ...
                const timeout = allowFallbackIfFailed(new Promise((resolve, reject) => {
                    if (this.options.openBrowserTimeout !== 0) {
                        this.timers.setTimeout
                            .call(null, () => reject(new types_1.MongoDBOIDCError('Opening browser timed out')), this.options.openBrowserTimeout ?? api_1.kDefaultOpenBrowserTimeout)
                            ?.unref?.();
                    }
                }));
    ...
    async initiateAuthAttempt(state, driverAbortSignal, { forceRefreshOrReauth = false } = {}) {
        ...
        try {
            get_tokens: {
                if (!forceRefreshOrReauth &&
                    tokenExpiryInSeconds(state.currentTokenSet?.set, passIdTokenAsAccessToken) >
                        5 * 60) {
        ...
        const { token_type, expires_at, access_token, id_token, refresh_token } = state.currentTokenSet.set;
        const tokenSetId = (0, util_1.getStableTokenSetId)(state.currentTokenSet.set);
        ...
        this.logger.emit('mongodb-oidc-plugin:auth-succeeded', {
            ...
            refreshToken: (0, util_1.getRefreshTokenId)(refresh_token),
    ...
    async requestToken(params) {
        ...
        if (params.refreshToken) {
            (state.discardingTokenSets ?? (state.discardingTokenSets = [])).push(params.refreshToken);
