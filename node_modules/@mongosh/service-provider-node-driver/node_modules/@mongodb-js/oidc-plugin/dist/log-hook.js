...
function hookLoggerToMongoLogWriter(emitter, log, contextPrefix) {
    ...
    emitter.on('mongodb-oidc-plugin:local-listen-resolved-hostname', (ev) => {
        log.info('OIDC-PLUGIN', mongoLogId(1002000028), `${contextPrefix}-oidc`, 'Resolved hostnames for local server', {
            ...ev,
        });
    });
    ...
    emitter.on('mongodb-oidc-plugin:open-browser-complete', () => {
        log.info('OIDC-PLUGIN', mongoLogId(1002000025), `${contextPrefix}-oidc`, 'Successfully opened browser');
    });
    ...
    emitter.on('mongodb-oidc-plugin:refresh-skipped', (ev) => {
        log.info('OIDC-PLUGIN', mongoLogId(1002000026), `${contextPrefix}-oidc`, 'Token refresh attempt skipped', { ...ev });
    });
    emitter.on('mongodb-oidc-plugin:refresh-started', (ev) => {
        log.info('OIDC-PLUGIN', mongoLogId(1002000018), `${contextPrefix}-oidc`, 'Token refresh attempt started', { ...ev });
    });
    emitter.on('mongodb-oidc-plugin:refresh-succeeded', (ev) => {
        log.info('OIDC-PLUGIN', mongoLogId(1002000019), `${contextPrefix}-oidc`, 'Token refresh attempt succeeded', { ...ev });
    });
    ...
    emitter.on('mongodb-oidc-plugin:request-token-started', (ev) => {
        log.info('OIDC-PLUGIN', mongoLogId(1002000029), `${contextPrefix}-oidc`, 'Request token started', { ...ev });
    });
    emitter.on('mongodb-oidc-plugin:request-token-ended', (ev) => {
        log.info('OIDC-PLUGIN', mongoLogId(1002000030), `${contextPrefix}-oidc`, 'Request token finished', { ...ev });
    });
    emitter.on('mongodb-oidc-plugin:discarding-token-set', (ev) => {
        log.info('OIDC-PLUGIN', mongoLogId(1002000031), `${contextPrefix}-oidc`, 'Discarding token set', { ...ev });
    });
    ...
    emitter.on('mongodb-oidc-plugin:state-updated', (ev) => {
        log.info('OIDC-PLUGIN', mongoLogId(1002000027), `${contextPrefix}-oidc`, 'Updated internal token store state', { ...ev });
    });
}
