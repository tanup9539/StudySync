...
async function runSmokeTests({ smokeTestServer, args: [executable, ...args], wantPerformanceTesting, }) {
    ...
    const expectAutomaticEncryptionSupport = !process.env.MONGOSH_NO_AUTOMATIC_ENCRYPTION_SUPPORT;
    if (!wantPerformanceTesting) {
    ...
    const perfResults = [];
    for (const { name, tags, input, output, env, testArgs, includeStderr, exitCode, perfTestIterations, } of [
        {
      ...
      db.dropDatabase();`,
        ...
        for (const [index, arg] of testArgs.entries()) {
            if (arg.includes('$INPUT_AS_FILE')) {
                ...
                await fs_1.promises.writeFile(tmpfile, Array.isArray(input) ? input.join('\n') : input, { mode: 0o600, flag: 'wx' });
...
async function runSmokeTest({ name, executable, args, env, input, output, exitCode, includeStderr, printSuccessResults, }) {
    var _a;
    ...
    try {
        ...
        assert_1.default.doesNotMatch(stderr, /ExperimentalWarning/);
